...
function testMakeMiddleEarth() {
  if (typeof makeMiddleEarth === "function") {
    if (makeMiddleEarth() === "Hello Middle Earth!") {
      ...
      console.log(
        "Your makeMiddleEarth function is returning the wrong value. Check for typos!"
      );
    }
  } else {
...
function testKeepSecret() {
  if (typeof keepSecret === "function") {
    if (
      keepSecret("FRODO") === "frodo" &&
      keepSecret("THE RING") === "the ring"
    ) {
      keepItSecretKeepItSafe();
      console.log(
        "Task 3 completed! Frodo has the ring. Click on it to hear something sinister..."
      );
    } else {
...
function testDoubleParty() {
  if (typeof doubleParty === "function") {
    if (doubleParty() === 8) {
      ...
      clickableStrider();
      console.log(
        "Task 4 completed! Your friends are in Rivendell. Click on Strider to reveal his true nature!"
      );
    } else {
...
function testDeclareAllegiance() {
  if (typeof declareAllegiance === "function") {
    if (declareAllegiance("Frodo") === "I, Frodo, am part of the fellowship!") {
      console.log(
        "Task 6 completed! The fellowship has been forged! Click on Rivendell to hear a tune!"
      );
      forgeTheFellowShip();
      ...
      console.log(
        "Your declareAllegiance function is returning the wrong value. Check for typos!"
      );
    }
  } else {
...
function testSlayTheBalrog() {
  if (typeof slayTheBalrog === "function") {
    if (
      slayTheBalrog() ===
      "Gandalf the Grey says: You shall not pass! Gandalf the Grey is now Gandalf the white."
    ) {
      console.log(
        "Task 7 completed! Gandalf has slayed the Balrog, check out his new name!"
      );
      theBalrog();
    } else {
      console.log(
        "Your slayTheBalrog function is returning the wrong value. Check for typos!"
      );
    }
  } else {
...
function testBlowTheHornOfGondor() {
  if (typeof blowTheHornOfGondor === "function") {
    if (blowTheHornOfGondor() === "Boromir has blown the Horn of Gondor!") {
      console.log(
        "Task 8 completed! Boromir has blown the horn...and been killed by the Uruk-hai!"
      );
      hornOfGondor();
    } else {
      console.log(
        "Your blowTheHornOfGondor function is returning the wrong value. Check for typos!"
      );
    }
  } else {
...
function testItsDangerousToGoAlone() {
  if (typeof itsDangerousToGoAlone === "function") {
    if (
      ...
      console.log(
        "Task 9 completed! Frodo and Sam have entered Mordor. Mount Doom is near!"
      );
      goToMordor();
    } else {
      console.log(
        "Your itsDangerousToGoAlone function is returning the wrong value. Check for typos!"
      );
    }
  } else {
...
function testWeWantsIt() {
  if (typeof weWantsIt === "function") {
    if (gollumHasRing) {
      ...
      console.log(
        "Gollum does not have the ring yet. Check your variable scope!"
      );
    }
  } else {
...
function testWrestleGollum() {
  if (typeof wrestleGollum === "function") {
    if (wrestleGollum() === 5) {
      console.log(
        "Task 11 completed! Gollum has been vanquished and the ring destroyed!"
      );
      victoryLapGoneWrong();
    } else {
...
function testThereAndBackAgain() {
  ...
  if (typeof thereAndBackAgain === "function") {
    if (
      ...
      console.log(
        "Task 12 completed! The hobbits have made it safely back to The Shire and saved Middle Earth!"
      );
      backToTheShire();
    } else {
      console.log(
        "Your thereAndBackAgain function is returning the wrong value"
      );
    }
  } else {
